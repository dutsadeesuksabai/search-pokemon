...
import { memo, useCallback, useMemo, Suspense, useState } from 'react';
...
const PokemonDisplay = ({ pokemonDetail,pokemonEvo }:{pokemonDetail: Pokemon ; pokemonEvo: PokemonEvolution[] | null;}) => {
...
const SearchForm = () => {
  ...
  const [search, setSearch] = useState(searchParams.get('name')?.toLowerCase() || '');
...
const SearchParamsWrapper = () => {
  ...
  const pokemonDetail = pokemonData?.pokemon || null;
  const pokemonEvo = evoData?.pokemon?.evolutions || null;
  ...
  return (
    ...
    <Suspense fallback={<SkeletonLoader />}>
      <PokemonDisplay pokemonDetail={pokemonDetail} pokemonEvo={pokemonEvo} />
    </Suspense>
  ) : (
    <>
    {
